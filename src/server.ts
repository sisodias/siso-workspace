...
export function createServer(
  ...
  const fleet = process.env.SISO_FLEET_CONFIG
    ? createFleet(app, process.env.SISO_FLEET_CONFIG, config.stateDir)
    : undefined;
  const mcpUrl = new URL("/mcp", config.publicBaseUrl);
  const resourceServerUrl = resourceUrlFromServerUrl(mcpUrl);
  const profilePaths = Object.keys(fleet?.profiles ?? {}).map(name => `/mcp/${name}`);
  const sessionResources = new WeakMap<Transport, string>();
  const oauthProvider = new SingleUserOAuthProvider(config.oauth, mcpUrl, config.stateDir, profilePaths.map(path => new URL(path, config.publicBaseUrl)));
  const authFor = (resource: URL) => requireBearerAuth({
  ...
  app.use(
    mcpAuthRouter({
      ...
      resourceName: fleet ? "SISO Workspace" : "DevSpace",
    }),
  );
  ...
  for (const path of profilePaths) {
    ...
    app.get(new URL(getOAuthProtectedResourceMetadataUrl(resource)).pathname, (_req, res) => {
      res.set("Access-Control-Allow-Origin", "*").json({resource: resource.href,
        authorization_servers: [new URL(config.publicBaseUrl).href], scopes_supported: config.oauth.scopes,
        bearer_methods_supported: ["header"], resource_name: fleet!.profiles[path.slice(5)].title});
    });
  }
  ...
  app.get("/healthz", (_req, res) => {
    res.json({ ok: true, name: fleet ? "siso-workspace" : "devspace" });
  });

  app.all(["/mcp", ...profilePaths], async (req, res) => {
    const requestedResource = new URL(req.path, config.publicBaseUrl);
    const profile = req.path === "/mcp" ? undefined : fleet?.profiles[req.path.slice(5)];
    const requestId = res.locals.requestId as string | undefined;
    const sessionId = req.header("mcp-session-id");
    const initializeRequest = req.method === "POST" && isInitializeRequest(req.body);

    await new Promise<void>((resolve, reject) => {
      // Resolve on an auth rejection too: the middleware has already ended the response.
      res.once("finish", resolve);
      authFor(requestedResource)(req, res, (error?: unknown) => {
    ...
    if (!req.auth?.resource || req.auth.resource.href !== requestedResource.href) {
    ...
    try {
      ...
      if (sessionId) {
        ...
        if (sessionResources.get(transport) !== requestedResource.href) {
          sendJsonRpcError(res, 403, -32001, "Session belongs to a different MCP resource");
          return;
        }
        ...
        sessionResources.set(transport, requestedResource.href);
        ...
        const server = fleet ? fleet.server(profile?.node, profile?.title) : createMcpServer(
          config,
          workspaces,
          reviewCheckpoints,
          processSessions,
